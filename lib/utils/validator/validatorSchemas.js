...
const userUpdateSchema = Joi.object({
    name: Joi.string().min(5).max(100).required(),
    pass: Joi.string().min(8).max(20).optional(),
    about: Joi.string().min(100).max(500),
    gender: Joi.string().valid('male', 'female', 'other').required(),
    image: Joi.string().max(1000).uri().required(),
    bannerImg: Joi.string().max(1000).uri().required(),
})

// user schema - for update by admin or moderator
...
const searchDataSchema = Joi.object({
    search: Joi.string().min(10).max(100).required(),
})
