...
const userService = {
    ...
    getAllUsers: async (req) => {
        const loggedUserRole = req?.roles // logged in user role
        if (!loggedUserRole) {
            throw new ApiError(401, "Please login to get all users")
        }

        const isAdmin = loggedUserRole?.includes(roles.ADMIN)
        const isModerator = loggedUserRole?.includes(roles.MODERATOR)
        ...
        const userRole = role // for filtering users by role
        ...
        if(verified !== "all") query.isVerified = verified
        if(approved !== "all") query.isApproved = approved
        if(blocked !== "all") query.isBlocked = blocked
    ...
    searchUsers: async (req) => {
        ...
        // search data - user name or email
        const searchData = await req?.json()
        console.log(searchData)
        
        if(!searchData?.query) throw new ApiError(400, "Nothing to search")
        
        const pageNum = parseInt(page) || 1
        const pageLimit = 40
        const skip = (pageNum - 1) * pageLimit
        
        const searchRegex = new RegExp(searchData?.query, "i")
        // get the user data
        ...
        return new ApiResponse(200, {users, totalPages: Math.ceil(total / pageLimit)}, "Users found")
