...
import { tagModel, blogModel } from "@/lib/models/index"
...
const tagService = {
    ...
    save: async (req, context) => {
        ...
        if (validate?.error) {
            throw new ApiError(400, validate.error.message.replace(/\"/g, ''))
        }
        ...
        return new ApiResponse(200, saveTag, "Tags saved successfully")
    },
