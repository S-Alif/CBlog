...
const getBlogs = async (query, sortOptions = { createdAt: -1 }, skip, limit) => {
    const blogs = await blogModel.find(query)
        ...
        .skip(skip)
        .limit(limit)
        .select("-blogContent -isBlocked -isPublished -createdAt -updatedAt")
...
const blogService = {
    ...
    saveBlog: async (req, context) => {
        ...
        const blogExists = await blogModel.findOne({ _id: blogId, authorId: userId }).lean()
        if (blogId && !blogExists) {
            throw new ApiError(404, 'Blog not found or you are not the author of this blog')
        }

        // if it is a new blog, set the authorId
        if (!blogId) {
            data.authorId = userId
        }

        const saveBlog = await blogModel.findByIdAndUpdate(
            { _id: blogId ? blogId : new mongoose.Types.ObjectId(), authorId: userId },
            data,
            {
                new: true,
    ...
    removeBlog: async (req, context) => {
        ...
        const blog = await blogModel.findById({ _id: blogId }).select('authorId').lean()
    ...
    getAllBlogs: async (req) => {
        ...
        const { page, limit = 60, sort = "recent", category = "all", tag = "all", published = "all", blocked = "all" } = getSearchParams(req)
